...
def Generate(member):
    ...
    limit_birth = datetime.datetime.now() - datetime.timedelta(days=1)
    query = models.Notification.query
    notifications_old = query.filter_by(receiver=member).filter(
        models.Notification.timestamp < limit).all()
    ...
    notifications = query.filter_by(receiver=member).filter(
        models.Notification.object_type != 9).filter(
            models.Notification.timestamp > limit).all()

    noti_births = query.filter_by(receiver=member).filter(
        models.Notification.object_type == 9).filter(
            models.Notification.timestamp > limit_birth).all()
    ...
    #    task_members = collections.defaultdict(set)
    #    task_comment_time = collections.defaultdict(int)
    #    task_comment_members = collections.defaultdict(set)

    for noti in notifications:
        if noti.object_type == models.ObjectType.User:
            ...
            descriptions.append(
                NotificationDescription(
                    ...
                    (noti.sender.cycle, noti.sender.nickname), noti.timestamp,
                    link, 'fa-user'))
        elif noti.object_type == models.ObjectType.Birthday:
            if noti.object_id in member_set:
                continue
            member_set |= {noti.object_id}
            link = '/stem/people/%d' % noti.sender.id
            descriptions.append(
                NotificationDescription(
                    ...
                    (noti.sender.birthday.strftime('%m/%d'), noti.sender.cycle,
                     noti.sender.nickname), noti.timestamp, link,
                    'fa-birthday-cake'))


#       elif noti.object_type == models.ObjectType.Task:
...
#       elif noti.object_type == models.ObjectType.TaskComment:
